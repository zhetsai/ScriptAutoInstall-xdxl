...
function ovpn_install() {
    rm -rf /etc/openvpn
    mkdir -p /etc/openvpn
    wget -O /etc/openvpn/vpn.zip "https://raw.githubusercontent.com/zhets/ScriptAutoInstall-xdxl/main/vpn.zip" >/dev/null 2>&1 
...
cd /var/www/html/
zip xdxl-openvpn.zip tcp.ovpn udp.ovpn ssl.ovpn ws-ssl.ovpn > /dev/null 2>&1
cd
...
<li class="list-group-item justify-content-between align-items-center" style="margin-bottom:1em;"><p> ALL.zip <span class="badge light-blue darken-4">Android/iOS/PC/Modem</span><br /><small></small></p><a class="btn btn-outline-success waves-effect btn-sm" href="https://IP-ADDRESSS:81/xdxl-openvpn.zip" style="float:right;"><i class="fa fa-download"></i> Download</a></li>
